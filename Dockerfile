...
RUN apt-get update &&\
	apt-get install --yes sudo curl tar ssh pdsh openjdk-8-jdk vim
...
RUN useradd --create-home --shell /bin/bash hadoop &&\
	...
	passwd -d hadoop
#RUN useradd --create-home --shell /bin/bash --gid hadoop hdfs
#RUN useradd --create-home --shell /bin/bash --gid hadoop yarn
...
#ADD spark-3.0.2-bin-hadoop2.7.tgz .
#ADD hadoop-2.7.4.tar.gz .
